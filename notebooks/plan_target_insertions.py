...
from aind_mri_utils.rotations import apply_transform_to_trimesh
...
from aind_mri_utils.optimization import get_headframe_hole_lines,create_rigid_transform,append_ones_column
...
from aind_mri_utils.utils import prepare_data_for_homogeneous_transform,extract_data_for_homogeneous_transform,make_homogeneous_transform
...
probe_model_file = headframe_model_dir /'dovetailtweezer_oneShank_centered_corrected.obj'#"modified_probe_holder.obj"
annotations_path = base_dir / "ephys/persist/data/MRI/processed/{}/UW2".format(mouse)
...
point_dictinary = {
    ...
    'LC': np.array([6637.28,4265.02,10707.35,]),
...
hmg_pts = prepare_data_for_homogeneous_transform(np.array(tuple(prefered_pts.values())))
chem_shift_annotation = np.dot(np.dot(hmg_pts,chem_shift_trans),trans)
transformed_annotation = extract_data_for_homogeneous_transform(chem_shift_annotation)
...
transformed_brain = extract_data_for_homogeneous_transform(np.dot(np.dot(prepare_data_for_homogeneous_transform(brain_pos), chem_shift_trans),trans))
transformed_implant = extract_data_for_homogeneous_transform(np.dot(prepare_data_for_homogeneous_transform(implant_targets),trans))
...
for tt in range(transformed_prefered.shape[0]):
    for hh in range(transformed_implant.shape[0]):
        insertion_vector = transformed_implant[hh, :] - transformed_prefered[tt, :]
        ap, ml = arc_angles_to_hit_two_points(
            transformed_prefered[tt, :], transformed_implant[hh, :], ap_offset=0
        )
...
mesh = load_newscale_trimesh(probe_model_file,move_down=.5
)
...
meshes = [
    trimesh.creation.uv_sphere(radius=0.25) for i in range(len(transformed_prefered))
]
for i, m in enumerate(meshes):
...
mesh = load_newscale_trimesh(probe_model_file,-0.2, )
...
for this_angle in angle_sets:
    ...
    for this_insertion in range(len(match_insertions)):
        ...
        TB = transform_matrix_from_angles_and_target(
            df.ap[insert_list[this_insertion]], -df.ml[insert_list[this_insertion]], df.target_loc[insert_list[this_insertion]]
