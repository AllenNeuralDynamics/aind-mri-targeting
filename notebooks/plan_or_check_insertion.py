...
class Probe:
    probe_type: str
    pose: ProbePose
    probe_info_name: Optional[str] = None
    ...
    def from_app_data(cls, data: AppData, probe_name: str, calibration: Optional[AffineTransform] = None) -> "Probe":
        ...
        probe_info_name = probe_name if calibration is not None else None
        probe_type = data.probe_config.probe_info[probe_name].type
        return cls(
            probe_type=probe_type, pose=probe_pose, probe_info_name=probe_info_name, calibration_transform=calibration
        )
...
class ProbeWidgetController:
    ...
    # Core widgets
    structure_dd: widgets.Dropdown = field(init=False)
    ...
    def __post_init__(self):
        ...
        self.controls = widgets.VBox(
            [
                widgets.HBox([self.structure_dd, self.arc_label, self.arc_assign_dd, self.target_dd, self.goto_btn]),
                kb_help,
                self.kb_panel,
    ...
    def _build_widgets(self):
        self.structure_dd = widgets.Dropdown(
    ...
    def _populate_initial(self):
        ...
        if self.structure_dd.options:
            self.structure_dd.value = list(self.structure_dd.options)[0]
            self._load_probe_into_widgets(self.structure_dd.value)
